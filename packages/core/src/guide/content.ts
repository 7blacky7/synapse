...
export const TOOL_GUIDES: Record<string, ToolGuide> = {
  ...
  workspace: {
    ...
    param_tips: 'name: Ziel-Workspace (Default main; main behaelt den DNS-Namen ohne Suffix). configure wirkt erst beim naechsten Container-Start. Jede Antwort liefert dns_name — IMMER den DNS-Namen nutzen, NIE die Container-IP (wechselt bei Restart). role: wirkt nur bei ERST-Anlage einer Instanz (Template-Werte werden in die Row kopiert; danach configure nutzen); init_timeout_ms konfiguriert lange Rollen-Provisionierungen; Template-Edits wirken ab dem naechsten Container-Start der Instanzen.',
    examples: [
