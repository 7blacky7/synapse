...
export const TOOL_GUIDES: Record<string, ToolGuide> = {
  ...
  files: {
    ...
    actions: {
      ...
      history: {
        description: 'Audit-Log: chronologische Liste aller Datei-Aenderungen mit Begruendung. Crash-Recovery: nach Session-Crash kann eine neue Session sehen "wer hat wann was warum geaendert" und gegebenenfalls Versionen wiederherstellen. IDEA-3a: Filter nach feature_tag (exakter Match) oder version_id (rekursive parent-chain via parent_version_id).',
        params: 'project (req); optional: agent_id (Filter), file_path (Praefix-Match), since (ISO-Timestamp), limit (Standard 50, Max 500), feature_tag (IDEA-3a, exakter Match), version_id (IDEA-3a, BIGINT als String — liefert die Korrektur-Chain ab dieser Version rekursiv).',
        example: 'files({ action: "history", project: "synapse", feature_tag: "idea-thought-task-link", limit: 20 })',
        tips: 'reason wird beim Schreiben mitgegeben (files write-actions + plan/commit). Eintraege enthalten file_path, edit_action, agent_id, batch_id, reason, created_at und IDEA-3a: feature_tag, parent_version_id, git_commit_sha. Voller Inhalt einer Version: files(action: "get_version", version_id). version_id-Filter ist projektgebunden (Cross-Project-Schutz).',
      },
    },
  },
  ...
  thought: {
    ...
    actions: {
      add: {
        description: 'Neuen Thought speichern. IDEA-1: optional task_id + task_status — verknuepft den Thought mit einer Plan-Task UND aktualisiert deren Status in einem Call (kein separater plan(update_task) noetig).',
      ...
      add_batch: {
        description: 'Mehrere Thoughts atomar speichern (1..50 Items in einem Call). IDEA-1: items[].task_id pro Item moeglich; top-level task_status setzt einmalig den Status fuer alle in den Items verlinkten Tasks.',
