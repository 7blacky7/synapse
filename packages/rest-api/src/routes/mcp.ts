...
const MCP_TOOLS = [
  ...
  {
    name: 'workspace',
    description: 'Pro-Projekt isolierter Docker-Sandbox-Container (synapse-workspace:latest) auf dem Synapse-Server. EINFACHSTE NUTZUNG: workspace({action:"exec", project:"<name>", command:"<shell>"}). Der Container wird beim ersten exec automatisch erstellt + gestartet (Lazy-Start). NORMALER WORKFLOW: schreibe Files via files-Tool in code_files → werden via PG-LISTEN/NOTIFY automatisch in den Container synchronisiert (auto-sync), keine manuelle materialize noetig. exec startet sofort, Container sieht neue Files in <1s. Lifecycle: Idle-Stop nach 10 Min, LRU-Eviction bei Cap. Actions: exec (Hauptanwendung, Shell-Kommando), list (Status aller), start (manuell, selten noetig — exec startet auto), stop (Container stoppen, Volume bleibt), pin/unpin (vor LRU schuetzen), materialize (FULL sync PG→Container, nuetzlich bei Container-Wiederstart), commit (SELTEN — nur als Notfall wenn Shell wirklich neue Files erzeugt die in PG sollen, z.B. nach \"npm pkg set\"). exec laeuft als user synapse(1000), WorkingDir /workspace, Default-Timeout 60s.',
    inputSchema: {
      type: 'object',
      properties: {
