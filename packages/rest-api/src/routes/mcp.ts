...
const MCP_TOOLS = [
  ...
  {
    name: 'workspace',
    description: 'Pro-Projekt isolierter Docker-Sandbox-Container (synapse-workspace:latest) auf dem Synapse-Server. EINFACHSTE NUTZUNG: workspace({action:"exec", project:"<name>", command:"<shell>"}). Container wird beim ersten exec automatisch erstellt + gestartet (Lazy-Start). NORMALER WORKFLOW: schreibe Files via files-Tool in code_files → werden via PG-LISTEN/NOTIFY automatisch in den Container synchronisiert (auto-sync); files-Tool DELETE → wird auch im Container geloescht. exec startet sofort, Container sieht neue Files in <1s. ⚠️ WICHTIG SHELL-WRITES: Source-Files im /workspace sind READ-ONLY (mode 0444, root-owned). "echo > foo.ts", "sed -i", "vim :w" laufen ins Permission denied. Aenderungen an Source-Code MUESSEN ueber files-Tool laufen (auto-syncs hierher). Shell ist NUR fuer install/build/test (npm install schreibt /workspace/node_modules, builds /workspace/dist — diese Verzeichnisse bleiben schreibbar). Lifecycle: Idle-Stop nach 10 Min, LRU-Eviction bei Cap. Actions: exec (Hauptanwendung), list, start (manuell, selten noetig), stop, pin/unpin, materialize (FULL sync PG→Container bei Wiederstart), commit (DEPRECATED — files-Tool nutzen). exec laeuft als user synapse(1000), WorkingDir /workspace, Default-Timeout 60s.',
    inputSchema: {
      type: 'object',
      properties: {
